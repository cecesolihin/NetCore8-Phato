...
namespace ThePatho.Features.Global.Nationality.Service
{
    public class NationalityService : INationalityService
    {
        ...
        public async Task<ApiResponse<AttachmentFileDto>> ExportAsync(ExportNationalityCommand request)
        {
            try
            {
                var criteriaRequest = new GetNationalityByCriteriaCommand();
                var response = await GetNationalityByCriteria(criteriaRequest);
                var data = response.Data?.NationalityList ?? new System.Collections.Generic.List<NationalityDto>();
                ...
                if (type == "excel" || type == "xlsx")
                {
                    ...
                    var worksheet = workbook.Worksheets.Add("NationalityList");

                    // Title
                    worksheet.Cell("A1").Value = "Nationality List";
                    ...
                    var properties = typeof(NationalityDto).GetProperties();
                    int col = 2;
            ...
            {
                return new ApiResponse<AttachmentFileDto>(System.Net.HttpStatusCode.BadRequest, default, "Gagal export Nationality", ex.Message);
            }
        }
    }
}
