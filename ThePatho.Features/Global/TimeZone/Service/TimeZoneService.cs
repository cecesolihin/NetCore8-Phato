...
namespace ThePatho.Features.Global.TimeZone.Service
{
    public class TimeZoneService : ITimeZoneService
    {
        ...
        public async Task<ApiResponse<AttachmentFileDto>> ExportAsync(ExportTimeZoneCommand request)
        {
            try
            {
                var criteriaRequest = new GetTimeZoneByCriteriaCommand();
                var response = await GetTimeZoneByCriteria(criteriaRequest);
                var data = response.Data?.TimeZoneList ?? new System.Collections.Generic.List<TimeZoneDto>();
                ...
                if (type == "excel" || type == "xlsx")
                {
                    ...
                    var worksheet = workbook.Worksheets.Add("TimeZoneList");

                    // Title
                    worksheet.Cell("A1").Value = "TimeZone List";
                    ...
                    var properties = typeof(TimeZoneDto).GetProperties();
                    int col = 2;
            ...
            {
                return new ApiResponse<AttachmentFileDto>(System.Net.HttpStatusCode.BadRequest, default, "Gagal export TimeZone", ex.Message);
            }
        }
    }
}
