...
namespace ThePatho.Features.Global.RadiusUnit.Service
{
    public class RadiusUnitService : IRadiusUnitService
    {
        ...
        public async Task<ApiResponse<AttachmentFileDto>> ExportAsync(ExportRadiusUnitCommand request)
        {
            try
            {
                var criteriaRequest = new GetRadiusUnitByCriteriaCommand();
                var response = await GetRadiusUnitByCriteria(criteriaRequest);
                var data = response.Data?.RadiusUnitList ?? new System.Collections.Generic.List<RadiusUnitDto>();
                ...
                if (type == "excel" || type == "xlsx")
                {
                    ...
                    var worksheet = workbook.Worksheets.Add("RadiusUnitList");

                    // Title
                    worksheet.Cell("A1").Value = "RadiusUnit List";
                    ...
                    var properties = typeof(RadiusUnitDto).GetProperties();
                    int col = 2;
            ...
            {
                return new ApiResponse<AttachmentFileDto>(System.Net.HttpStatusCode.BadRequest, default, "Gagal export RadiusUnit", ex.Message);
            }
        }
    }
}
