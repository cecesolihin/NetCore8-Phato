...
namespace ThePatho.Features.Global.InventoryGroupOrg.Service
{
    public class InventoryGroupOrgService : IInventoryGroupOrgService
    {
        ...
        public async Task<ApiResponse<AttachmentFileDto>> ExportAsync(ExportInventoryGroupOrgCommand request)
        {
            try
            {
                var criteriaRequest = new GetInventoryGroupOrgByCriteriaCommand();
                var response = await GetInventoryGroupOrgByCriteria(criteriaRequest);
                var data = response.Data?.InventoryGroupOrgList ?? new System.Collections.Generic.List<InventoryGroupOrgDto>();
                ...
                if (type == "excel" || type == "xlsx")
                {
                    ...
                    var worksheet = workbook.Worksheets.Add("InventoryGroupOrgList");

                    // Title
                    worksheet.Cell("A1").Value = "InventoryGroupOrg List";
                    ...
                    var properties = typeof(InventoryGroupOrgDto).GetProperties();
                    int col = 2;
                    ...
                    var dto = new AttachmentFileDto
                    {
                        ...
                        FileName = "InventoryGroupOrgList.xlsx",
                ...
                else if (type == "pdf")
                {
                    var doc = Document.Create(container =>
                    {
                        container.Page(page =>
                        {
                            ...
                            page.Header().Element(header =>
                            {
                                header.AlignCenter()
                                    .PaddingBottom(10)
                                    .Text("InventoryGroupOrg List Data")
                                    .SemiBold()
                                    .FontSize(18)
                                    .FontColor("#007BFF");
                            });
                    ...
                    {
                        Base64Data = Convert.ToBase64String(bytes),
                        FileName = "InventoryGroupOrgList.pdf",
            ...
            {
                return new ApiResponse<AttachmentFileDto>(System.Net.HttpStatusCode.BadRequest, default, "Gagal export InventoryGroupOrg", ex.Message);
            }
        }
    }
}
