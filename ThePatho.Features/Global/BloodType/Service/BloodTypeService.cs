...
namespace ThePatho.Features.Global.BloodType.Service
{
    public class BloodTypeService : IBloodTypeService
    {
        ...
        public async Task<ApiResponse<AttachmentFileDto>> ExportAsync(ExportBloodTypeCommand request)
        {
            try
            {
                var criteriaRequest = new GetBloodTypeByCriteriaCommand();
                var response = await GetBloodTypeByCriteria(criteriaRequest);
                var data = response.Data?.BloodTypeList ?? new System.Collections.Generic.List<BloodTypeDto>();
                ...
                if (type == "excel" || type == "xlsx")
                {
                    ...
                    var worksheet = workbook.Worksheets.Add("BloodTypeList");

                    // Title
                    worksheet.Cell("A1").Value = "BloodType List";
                    ...
                    var properties = typeof(BloodTypeDto).GetProperties();
                    int col = 2;
            ...
            {
                return new ApiResponse<AttachmentFileDto>(System.Net.HttpStatusCode.BadRequest, default, "Gagal export BloodType", ex.Message);
            }
        }
    }
}
