...
namespace ThePatho.Features.Global.ShoeSize.Service
{
    public class ShoeSizeService : IShoeSizeService
    {
        ...
        public async Task<ApiResponse<AttachmentFileDto>> ExportAsync(ExportShoeSizeCommand request)
        {
            try
            {
                var criteriaRequest = new GetShoeSizeByCriteriaCommand();
                var response = await GetShoeSizeByCriteria(criteriaRequest);
                var data = response.Data?.ShoeSizeList ?? new System.Collections.Generic.List<ShoeSizeDto>();
                ...
                if (type == "excel" || type == "xlsx")
                {
                    ...
                    var worksheet = workbook.Worksheets.Add("ShoeSizeList");

                    // Title
                    worksheet.Cell("A1").Value = "ShoeSize List";
                    ...
                    var properties = typeof(ShoeSizeDto).GetProperties();
                    int col = 2;
            ...
            {
                return new ApiResponse<AttachmentFileDto>(System.Net.HttpStatusCode.BadRequest, default, "Gagal export ShoeSize", ex.Message);
            }
        }
    }
}
