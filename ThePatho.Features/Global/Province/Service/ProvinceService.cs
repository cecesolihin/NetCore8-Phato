...
namespace ThePatho.Features.Global.Province.Service
{
    public class ProvinceService : IProvinceService
    {
        ...
        public async Task<ApiResponse<AttachmentFileDto>> ExportAsync(ExportProvinceCommand request)
        {
            try
            {
                var criteriaRequest = new GetProvinceByCriteriaCommand();
                var response = await GetProvinceByCriteria(criteriaRequest);
                var data = response.Data?.ProvinceList ?? new System.Collections.Generic.List<ProvinceDto>();
                ...
                if (type == "excel" || type == "xlsx")
                {
                    ...
                    var worksheet = workbook.Worksheets.Add("ProvinceList");

                    // Title
                    worksheet.Cell("A1").Value = "Province List";
                    ...
                    var properties = typeof(ProvinceDto).GetProperties();
                    int col = 2;
            ...
            {
                return new ApiResponse<AttachmentFileDto>(System.Net.HttpStatusCode.BadRequest, default, "Gagal export Province", ex.Message);
            }
        }
    }
}
