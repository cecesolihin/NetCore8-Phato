...
namespace ThePatho.Features.Global.City.Service
{
    public class CityService : ICityService
    {
        ...
        public CityService(ApplicationDbContext _context, DapperContext _dappercontext, SqlQueryLoader _queryLoader, IDbConnection _dbConnection, ICurrentUserService _currentUserService)
        {
            context = _context;
        ...
        public async Task<ApiResponse<AttachmentFileDto>> ExportAsync(ExportCityCommand request)
        {
            try
            {
                var criteriaRequest = new GetCityByCriteriaCommand();
                var response = await GetCityByCriteria(criteriaRequest);
                var data = response.Data?.CityList ?? new System.Collections.Generic.List<CityDto>();
                ...
                if (type == "excel" || type == "xlsx")
                {
                    ...
                    var worksheet = workbook.Worksheets.Add("CityList");

                    // Title
                    worksheet.Cell("A1").Value = "City List";
                    ...
                    var properties = typeof(CityDto).GetProperties();
                    int col = 2;
            ...
            {
                return new ApiResponse<AttachmentFileDto>(System.Net.HttpStatusCode.BadRequest, default, "Gagal export City", ex.Message);
            }
        }
    }
}
