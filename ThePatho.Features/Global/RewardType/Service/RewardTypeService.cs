...
namespace ThePatho.Features.Global.RewardType.Service
{
    public class RewardTypeService : IRewardTypeService
    {
        ...
        public async Task<ApiResponse<AttachmentFileDto>> ExportAsync(ExportRewardTypeCommand request)
        {
            try
            {
                var criteriaRequest = new GetRewardTypeByCriteriaCommand();
                var response = await GetRewardTypeByCriteria(criteriaRequest);
                var data = response.Data?.RewardTypeList ?? new System.Collections.Generic.List<RewardTypeDto>();
                ...
                if (type == "excel" || type == "xlsx")
                {
                    ...
                    var worksheet = workbook.Worksheets.Add("RewardTypeList");

                    // Title
                    worksheet.Cell("A1").Value = "RewardType List";
                    ...
                    var properties = typeof(RewardTypeDto).GetProperties();
                    int col = 2;
            ...
            {
                return new ApiResponse<AttachmentFileDto>(System.Net.HttpStatusCode.BadRequest, default, "Gagal export RewardType", ex.Message);
            }
        }
    }
}
