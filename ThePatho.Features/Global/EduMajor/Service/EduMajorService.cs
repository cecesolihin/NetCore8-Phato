...
namespace ThePatho.Features.Global.EduMajor.Service
{
    public class EduMajorService : IEduMajorService
    {
        ...
        public async Task<ApiResponse<AttachmentFileDto>> ExportAsync(ExportEduMajorCommand request)
        {
            try
            {
                var criteriaRequest = new GetEduMajorByCriteriaCommand();
                var response = await GetEduMajorByCriteria(criteriaRequest);
                var data = response.Data?.EduMajorList ?? new System.Collections.Generic.List<EduMajorDto>();
                ...
                if (type == "excel" || type == "xlsx")
                {
                    ...
                    var worksheet = workbook.Worksheets.Add("EduMajorList");

                    // Title
                    worksheet.Cell("A1").Value = "EduMajor List";
                    ...
                    var properties = typeof(EduMajorDto).GetProperties();
                    int col = 2;
            ...
            {
                return new ApiResponse<AttachmentFileDto>(System.Net.HttpStatusCode.BadRequest, default, "Gagal export EduMajor", ex.Message);
            }
        }
    }
}
