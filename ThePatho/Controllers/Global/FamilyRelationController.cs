...
namespace ThePatho.Controllers.Global
{
    ...
    public class FamilyRelationController : ControllerBase
    {
        ...
        public async Task<IActionResult> ExportFamilyRelation([FromQuery] string type, CancellationToken cancellationToken)
        {
            var exportResponse = await mediator.Send(new ExportFamilyRelationCommand { Type = type }, cancellationToken);
