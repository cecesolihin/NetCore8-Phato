...
namespace ThePatho.Controllers.Global
{
    ...
    public class DiseaseCategoryController : ControllerBase
    {
        ...
        public async Task<IActionResult> ExportDiseaseCategory([FromQuery] string type, CancellationToken cancellationToken)
        {
            var exportResponse = await mediator.Send(new ExportDiseaseCategoryCommand { Type = type }, cancellationToken);
